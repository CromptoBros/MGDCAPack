Ponder.registry((ponder) => {
	ponder
		.create(['mekanism:boiler_valve', 'mekanism:boiler_casing'])
		.scene(
			'boiler_construction',
			...
			(scene, util) => {
				...
				scene.text(
					80,
					'The Thermoelectric Boiler uses Heat to turn Water into Steam.'
				)
				...
				scene.text(70, 'Fill the next layer with Superheating Elements.')
					.attachKeyFrame
				scene.idle(80)
				...
				scene
					.text(
						70,
						'Additional Heater layers are optional, but make it run faster and hold more Water.'
					)
					.attachKeyFrame()
				scene.idle(80)
				...
				scene
					.text(
						70,
						'Hollow layers (called Water Cavity layers) can optionally be included, to hold more Water.'
					)
					.attachKeyFrame()
				scene.idle(80)
				...
				scene
					.text(
						80,
						'Fill a layer with Pressure Dispersers to create a "Steam Catch layer." The entire layer must be filled.'
					)
					.attachKeyFrame()
				scene.idle(90)
				...
				scene
					.text(
						80,
						'Hollow layers (called Steam Cavity layers) can optionally be included, to hold more Steam'
					)
					.attachKeyFrame()
				scene.idle(90)
				...
				scene
					.text(
						60,
						'Replace two blocks on the casing on a Heater or Water Cavity layer with Valves.'
					)
					.attachKeyFrame()
				scene.idle(70)
				scene
					.text(40, 'One acts as a Water input...', [4, 2, 2])
					.placeNearTarget()
				scene.idle(50)
				scene
					.text(40, '...and the other as Heat input.', [2, 2, 4])
					.placeNearTarget()
				scene.idle(50)
				...
				scene.world.modifyBlock(
					[4, 6, 2],
					(state) => state.with('mode', 'output_steam'),
					false
				)
				scene
					.text(
						60,
						'Replace a block on the casing on a Steam Catch or Steam Cavity layer with another Valve.'
					)
					.attachKeyFrame()
				scene.idle(70)
				scene
					.text(50, 'This will act as the Steam output.', [4, 6, 2])
					.placeNearTarget()
				scene.idle(60)
