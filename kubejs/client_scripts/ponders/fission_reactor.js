	...
	ponder
		...
		.scene(
			...
			(scene, util) => {
				...
				scene.text(
					70,
					'The Fission Generator is a multiblock that generates a massive amount of heat.'
				)
				scene.idle(80)

				scene
					.text(50, 'It does this by coolant (like Water)...', [5.5, 7.5, 1.5])
					.placeNearTarget()
				scene.idle(60)
				scene
					.text(50, '...into something hotter (like Steam).', [3.5, 7.5, 1.5])
					.placeNearTarget()
				scene.idle(70)

				scene
					.text(50, 'Fueled by Fissile Fuel..', [5.5, 1.5, 1.5])
					.placeNearTarget()
				scene.idle(60)
				scene
					.text(
						80,
						'...it creates dangerous Radioactive Waste as a byproduct.',
						[3, 1, 1]
					)
					.placeNearTarget()
				scene.idle(90)
				...
				let setFrame = (block) => {
					for (let i of frame) {
						for (let j of i) {
							scene.world.setBlock(j, block, false)
						}
					}
				}
				...
				scene.text(
					80,
					'Fill the walls with Reactor Glass (or more Reactor Casing).'
				)
				scene.idle(20)
				...
				scene
					.text(70, 'Place a pillar of Fusion Fuel Assemblies anywhere inside')
					.attachKeyFrame()
				scene.idle(30)
				...
				scene
					.text(60, 'On top of the pillar, place a Control Rod Assembly.')
					.attachKeyFrame()
				scene.idle(20)
				scene.world.setBlock(
					[4, 5, 4],
				...
				scene.text(
					70,
					'There can be multiple sets of Control Rods, but they cannot touch sides.'
				)
				...
				scene
					.text(
						60,
						'Anywhere on any of the sides, place 4 Fission Reactor Ports:'
					)
					.attachKeyFrame()
				scene.idle(70)
				...
				scene
					.text(40, 'One for Coolant input', [5.5, 6.5, 2.5])
					.placeNearTarget()
				scene.idle(50)
				...
				scene.world.modifyBlock(
					[3, 6, 2],
					(s) => s.with('mode', 'output_coolant'),
					false
				)
				scene
					.text(40, 'One for Heated Coolant output', [3.5, 6.5, 2.5])
					.placeNearTarget()
				scene.idle(50)
				...
				scene.world.modifyBlock(
					[3, 2, 2],
					(s) => s.with('mode', 'output_waste'),
					false
				)
				scene
					.text(40, 'And one for Radioactive Waste output', [3.5, 2.5, 2.5])
					.placeNearTarget()
				scene.idle(50)
				...
				scene.text(
					100,
					'Running it is VERY dangerous though, so make sure to read the quests explaining it!'
				)
			}
		)
})
