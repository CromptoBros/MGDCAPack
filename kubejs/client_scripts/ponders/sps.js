		...
		.scene(
			...
			(scene, util) => {
				...
				scene
					.text(
						60,
						'Basically, six 7-wide circles of SPS Casing with stuff inside'
					)
					.attachKeyFrame()
				scene.idle(70)
				...
				scene.text(40, 'SPS Casing', [6.5, 0.51, 7.5]).placeNearTarget()
				scene.idle(50)

				for (let i of circle) {
					scene.world.setBlock(i, 'mekanism:structural_glass', false)
				}
				...
				for (let i of circle) {
					scene.world.setBlock(i, 'mekanism:sps_casing', false)
				}
				...
				scene.idle(10)
				scene
					.text(70, 'Three SPS Ports must be placed inside any of the circles.')
					.attachKeyFrame()
				scene.idle(80)

				scene
					.text(60, 'One for Polonium input', [9.5, 4.5, 6.5])
					.placeNearTarget()
				scene.idle(70)

				scene
					.text(60, 'One for energy input', [6.5, 1.5, 6.5])
					.placeNearTarget()
				scene.idle(70)

				scene
					.text(60, 'One for Antimatter output', [3.5, 4.5, 6.5])
					.placeNearTarget()
				scene.idle(70)

				scene
					.text(60, 'Any Port can be anywhere inside any circle.')
					.attachKeyFrame()
				scene.idle(20)
				...
				scene
					.text(
						60,
						'The Supercharged Coil has to be on the Port that accepts power.'
					)
					.attachKeyFrame()
				scene.idle(10)
				...
				scene.world.modifyBlock(
					[6, 4, 8],
					(s) => s.with('facing', 'north'),
					false
				)
				scene.world.setBlock([6, 4, 10], 'mekanism:creative_energy_cube', false)

				scene
					.text(50, 'The two can be placed on the sides as well')
					.attachKeyFrame()
				scene.idle(60)
				...
				scene.text(
					70,
					'The more powered Coils, the faster Antimatter is created!'
				)
				scene.idle(80)
